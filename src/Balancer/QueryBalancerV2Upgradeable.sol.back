...
contract QueryBalancerV2Upgradeable is UUPSUpgradeable {
    ...
    function get_pool_type(address pool) public view virtual returns (string memory pool_type) {
        ...
        try IBalancerPool(pool).name() returns (string memory _name) {
            try IBalancerPool(pool).symbol() returns (string memory _symbol) {
                if (
                    contains(_name, "Stable") || contains(_name, "STABLE") || contains(_symbol, "Stable")
    ...
    // 查找子字符串的位置
    function indexOf(string memory haystack, string memory needle) internal pure virtual returns (int256) {
        ...
        )
    {
        ...
        (address[] memory tokens, uint256[] memory balances,) =
            IVault(IBalancerPool(pool).getVault()).getPoolTokens(poolId);
        ...
        if (
            keccak256(bytes(poolType)) == keccak256(bytes("Stable"))
                || keccak256(bytes(poolType)) == keccak256(bytes("ComposableStable"))
        ) {
...
interface IFXPoolDeployer {
    function getFXPoolDetails(address _fxpoolAddr)
        external
        view
        returns (
            string memory name,
...
contract QueryBalancerV2UpgradeableV3 is QueryBalancerV2UpgradeableV2 {
    ...
    function is_xave(address pool) public view virtual returns (bool) {
        ...
        if (tokens.length == 2) {
            ...
            for (uint256 i = 0; i < fxPools.length; i++) {
        ...
        try IFXPoolDeployer(fx_pool_deployer()).getFXPoolDetails(pool) returns (
            string memory, address, address, uint256, uint256, uint256, uint256, uint256, uint256, uint256
        ) {
            return true;
        } catch {
            // Ignore exception and proceed to the next check
    ...
    function get_params_v2(address pool)
        public
        view
        virtual
        returns (
