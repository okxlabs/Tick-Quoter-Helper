...
library QueryEkuboTicksSuperCompact {
    ...
    int32 constant EKUBO_MIN_TICK = -88722835;
    int32 constant EKUBO_MAX_TICK = 88722835;

    function poolState(
        address core,
        bytes32 poolId
    ) internal view returns (SqrtRatio sqrtRatio, int32 tick, uint128 liquidity) {
        return ICore(payable(core)).poolState(poolId);
    }
    ...
    function poolTicks(
        ...
        bytes32 poolId,
        int32 tick
    ) internal view returns (int128 liquidityDelta, uint128 liquidityNet) {
        return ICore(payable(core)).poolTicks(poolId, tick);
    }

    function poolTickSpacing(PoolKey memory pk) internal pure returns (uint32) {
    ...
    function queryEkuboTicksSuperCompactByTokens(
        ...
        return queryEkuboTicksSuperCompactByPoolKey(core, poolkey);
    }
